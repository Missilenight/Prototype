...
public class VicsekAgent : MonoBehaviour
{
    public bool IS_LAND_ANIMAL = false;

    [SerializeField]
    VicsekBehaviour ATTACK_BEHAVIOUR;

    [SerializeField]
    VicsekBehaviour FEAR_BEHAVIOUR;
    ...
    public void UpdateBehaviour(
     // Agents
     List<VicsekAgent> agents,

     // Vicsek Properties
     float noise, float alignmentWeight, float nieghbourRadius, 
     
     // Additional Behaviours
     float attractionWeight, float repultionWeight, float repultionRadius,
     ...
     )
    {
        ...
        // Getters
        List<VicsekAgent> neighbours = GetNeighbours(FieldOfView, nieghbourRadius, agents);
        Targets = GetTargets(FieldOfView, attackDistance);
        Predators = GetPredators(FieldOfView, fearDistance);
        VicsekAgent followLeader = GetLeader(neighbours);
        ...
        // Calculate Noise
        float noiseRange = (isFeared ? FEAR_BEHAVIOUR.Noise : isAttack ? ATTACK_BEHAVIOUR.Noise : noise);
        float randomNoise = Random.Range(-noiseRange, noiseRange);
        
        // Calculate additional behaviours
        Vector3 repultion = CalculateRepulsion(neighbours, repultionRadius, Speed) * (isFeared ? FEAR_BEHAVIOUR.RepulsionWeight : isAttack ? ATTACK_BEHAVIOUR.RepulsionWeight : repultionWeight);
        Vector3 attraction = CalculateAttraction(neighbours) * (isFeared ? FEAR_BEHAVIOUR.AttractionWeight : isAttack ? ATTACK_BEHAVIOUR.AttractionWeight : attractionWeight);
        ...
        Vector3 direction = attraction + alignment + repultion + boundary + leader + fear + attack + avoidance;
        ...
        Velocity = new Vector3(Mathf.Cos(angle), Mathf.Sin(angle), Velocity.z);
    ...
    public List<VicsekAgent> GetNeighbours(float fieldOfView, float neighboursRadius, List<VicsekAgent> agents)
    {
        List<VicsekAgent> neighbours = new List<VicsekAgent>();
    ...
    public VicsekAgent GetLeader(List<VicsekAgent> neighbours)
    {
        VicsekAgent leader = null;

        foreach (VicsekAgent agent in neighbours)
        {
            if (agent.isLeader)
            {
                if (leader == null)
    ...
    // Behaviours
    public Vector3 CalculateAttraction(List<VicsekAgent> neighbours)
    {
        ...
        foreach (VicsekAgent agent in neighbours)
            attraction += agent.transform.position;

        attraction /= neighbours.Count;

        attraction = (attraction - transform.position).normalized;

        return attraction;
    }

    public Vector3 CalculateAlignment(List<VicsekAgent> neighbours)
    {
    ...
    public Vector3 CalculateRepulsion(List<VicsekAgent> neighbours, float separationRadius, float speed)
    {
    ...
    public Vector3 CalculateLeader(VicsekAgent leader)
    {
        ...
        for (int i = 0; i < rayCountPerDimension; i++)
        {
            for (int j = 0; j < rayCountPerDimension; j++)
            {
                ...
                if (Physics.Raycast(transform.position, rayDirection, out hit, avoidanceDistance))
                    if (hit.collider.gameObject.tag != "Vicsek")
